...
class TestStellarBurgerRegistration:
    def test_successful_registration(self, driver):
        ...
        enter = WebDriverWait(driver, 5).until(EC.visibility_of_element_located((By.XPATH, ".//h2[text() = 'Вход']"))).text
    ...
    def test_registration_with_invalid_password(self, driver):
        ...
        driver.find_element(*Locators.PASSWORD_FOR_REGISTRATION).send_keys(password)
        driver.find_element(*Locators.REGISTRATION_BUTTON).click()
        enter = WebDriverWait(driver, 5).until(EC.visibility_of_element_located((By.XPATH, "//p[text() = 'Некорректный пароль']"))).text
